<template>
	<div>
		<tab active-color="#1e50ae" defaultColor="#333" bar-active-color="#1e50ae" class="college" style="position: absolute;top: 0;left: 0;width: 100%;z-index: 100;">
			<tab-item v-for="(item, index) in tabList" :selected="indexCur == index" :key="index" @on-item-click="onIndexChange">{{item}}</tab-item>
		</tab>
		<swiper v-model="indexCur" :show-dots="false" @on-index-change="onIndexChange" height="100%" style="padding-top: 44px;box-sizing: border-box; height: 100%;">
			<swiper-item>
				<view-box body-padding-bottom="0" ref="viewBox1">
					<div>
						<event-template v-for="item in listData1" :data="item" type="1" :key="item.id" @click.native="toDetail_M(item.id, 1)">
				...
				<div class="g-empty" v-if="emptyObj.empty1"><div class="text">暂无相关数据</div></div>
			</swiper-item>
			<swiper-item>
				<view-box body-padding-bottom="0" ref="viewBox2">
					<div>
						<event-template v-for="item in listData2" :data="item" type="2" :key="item.id" @click.native="toDetail_M(item.id, 2)">
							<div slot="enrol">
								<div class="btn-box">
				...
				<div class="g-empty" v-if="emptyObj.empty2"><div class="text">暂无相关数据</div></div>
			</swiper-item>
			<swiper-item>
				<view-box body-padding-bottom="0" ref="viewBox3">
					<div>
						<event-template v-for="item in listData3" :data="item" type="3" :key="item.id" @click.native="toDetail_M(item.id, 3)">
					    	<div class="top-three" slot-scope="{firstName, secondName, thirdName}" slot="topThree">
					    		<div class="item"><div v-if="firstName">{{firstName}}</div><div v-else>暂无</div></div>
					    		<div class="item"><div v-if="secondName">{{secondName}}</div><div v-else>暂无</div></div>
					    		<div class="item"><div v-if="thirdName">{{thirdName}}</div><div v-else>暂无</div></div>
			...
			<swiper-item>
				<view-box body-padding-bottom="0" ref="viewBox4">
					<div>
						<event-template v-for="item in listData4" :data="item" type="4" :key="item.id" @toGroupMe="toGroupMe_M" @click.native="toDetail_M(item.id, 4)" />
						<div style="padding: 1px;">
							<load-more :show-loading="moreObj.more4" :tip="moreObj.more4 ? '正在加载' : '我是有底线的'" background-color="#f6f6f6"></load-more>
						</div>
					</div>
				</view-box>
				<div class="g-empty" v-if="emptyObj.empty4"><div class="text">暂无相关数据</div></div>
			</swiper-item>
		</swiper>
...
<script>
	...
	export default {
		...
		mounted(){
			...
			this.scrollInstance1 = new this.ScrollPlugin(viewBox1, viewBox1.getScrollBody(), viewBox1.getScrollBody().children[0], this.tabBox1, this.indexCur==0?true:false);//true自动加载一次数据
		...
		methods: {
			...
			getData(params){
				...
				this.$post('/api/app/match/findForPage.v1', params, {loading: false}).then(
					res => {
						let rows = res.rows
						...
						if(rows){
							if(params.page == 1){
								this['listData'+index] = rows;
							}else{
								this['listData'+index] = this['listData'+index].concat(rows);
							}
